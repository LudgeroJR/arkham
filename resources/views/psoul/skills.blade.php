...
@section('content')
    ...
    <div x-data="{
        ...
        <!-- Pesquisa -->
        <input type="text" x-model="search" placeholder="Pesquisar skill..."
        ...
        <!-- Modal -->
        <div x-show="showModal && modalSkill" x-transition.opacity
            ...
            <div @click.away="closeModal()"
                ...
                <div class="grid gap-6 md:grid-cols-2 grid-cols-1"
                    ...
                    <template x-if="showGeneral">
                        <div class="flex flex-col items-center mb-4">
                            <div
                                ...
                                <svg class="w-14 h-14 text-[#B81F1C]" fill="none" stroke="currentColor"
                                    viewBox="0 0 24 24">
                                    <circle cx="12" cy="12" r="10" stroke="#B81F1C" stroke-width="2"
                                        fill="#F8EE9A" />
                                    <text x="50%" y="55%" text-anchor="middle" fill="#B81F1C" font-size="18"
                                        font-family="Bebas Neue, Oswald, Arial" dy=".3em">SK</text>
                                </svg>
                            </div>
                            ...
                            <div class="flex gap-2 items-center mb-2">
                                ...
                                <span
                                    class="inline-flex items-center px-2 py-0.5 rounded bg-[#B81F1C] text-[#F8EE9A] text-xs font-bold">
                                    <img :src="'{{ asset('images/jogos/psoul/types') }}/' + modalSkill.type + '.png'"
                                        alt="" class="w-4 h-4 mr-1"> <span x-text="modalSkill.type"></span>
                                </span>
                                <span
                                    class="inline-flex items-center px-2 py-0.5 rounded bg-[#EDC416] text-[#B81F1C] text-xs font-bold border border-[#B81F1C] ml-4">
                                    <span>Power: </span>
                                    <span class="ml-1" x-text="modalSkill.power || '-'"></span>
                                </span>
                            </div>
                            <div class="text-[#680F0F] mb-2 text-center font-semibold" x-text="modalSkill.description">
                            </div>
                            <div class="mb-2 flex flex-wrap gap-2 justify-center"
                                x-show="modalSkill.ranges && modalSkill.ranges.length">
                                <template x-for="r in modalSkill.ranges" :key="r">
                                    <span
                                        class="inline-block bg-[#F8EE9A] text-[#C6241D] rounded px-2 py-0.5 text-xs font-bold border border-[#B81F1C]"
                                        x-text="r"></span>
                                </template>
                            </div>
                        </div>
                    ...
                    <template x-if="showMoveset">
                        <div>
                            <div
                                class="bg-white/90 rounded-xl shadow p-4 border-l-8 border-[#B81F1C] flex flex-col max-h-96 overflow-y-auto min-w-0">
                                ...
                                <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
                                    <template x-for="poke in modalSkill.moveset_pokemons" :key="poke.id">
                                        <div
                                            class="flex flex-col items-center bg-[#F8EE9A] rounded-lg p-2 shadow border border-[#EDC416] hover:bg-[#FFFBE6] transition">
                                            <img :src="'{{ asset('images/jogos/psoul/pokemonsthumb') }}/' + poke.thumb"
                                                :alt="poke.name"
                                                class="w-12 h-12 object-contain mb-1 rounded-full border-2 border-[#B81F1C] bg-white"
                                                loading="lazy">
                                            <span class="text-[#B81F1C] font-bold text-sm text-center"
                                                x-text="poke.name"></span>
                                        </div>
                                    </template>
                                </div>
                            </div>
                    ...
                    <template x-if="showEgg || showTutor">
                        <div class="flex flex-col gap-4">
                            <template x-if="showEgg">
                                <div
                                    class="bg-white/90 rounded-xl shadow p-4 border-l-8 border-[#B81F1C] flex flex-col max-h-44 overflow-y-auto min-w-0">
                                    <div class="font-extrabold text-[#B81F1C] mb-1 text-lg flex items-center gap-2">
                                        <span class="inline-block w-3 h-3 rounded-full bg-[#B81F1C]"></span>
